...
function willYouMarryMe(isPositiveAnswer) {
  const answ = new Promise((resolve, reject) => {
...
function chainPromises(array, action) {
  ...
  return Promise.resolve(array)
    .then((promices) => promices.forEach((prom) => {
      prom.then((data) => resArray.push(data)).catch(() => {});
    }))
    .then(() => resArray.reduce((res, item) => action(res, item)));
}
