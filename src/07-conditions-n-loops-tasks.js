...
function isCreditCardNumber(ccn) {
  ...
  const resArray = arr.map((elem, index) => {
    const flag = arr.length % 2 === 0 ? index % 2 === 0 : index % 2 !== 0;
    if (!flag) {
      return +elem;
    }
    const two = +elem * 2;
  ...
  const res = resArray.reduce((a, b) => a + b);
  return res % 10 === 0;
}
...
function getDigitalRoot(num) {
  ...
  const numarray = arr.map((elem) => Number(elem));
  const sum = numarray.reduce((total, elem) => total + elem, 0);
...
function getMatrixProduct(m1, m2) {
  const arr = Array(m1.length).fill([]);
...
function evaluateTicTacToePosition(position) {
  for (let i = 0; i < position.length; i += 1) {
    if (position[i][0] === position[i][1]
       && position[i][1] === position[i][2] && position[i][0] !== undefined) {
      return position[i][0];
    }
